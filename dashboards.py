...
# tipo de produtos mais vendido, contribuição por filial
dif_prod_date = px.bar(df_filtered, x="Date", y="Product line",
                       color="City", title="Faturamento por produto", orientation="h")
col2.plotly_chart(dif_prod_date, use_container_width=True)
...
city_rating_total = df_filtered.groupby(
    "City")[["Rating"]].mean().reset_index()
fig_rating = px.bar(city_rating_total, y="Rating", x="City",
                    title="Avaliação")
